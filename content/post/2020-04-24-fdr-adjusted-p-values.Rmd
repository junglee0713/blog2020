...
In this post, we look into the so-called FDR-adjusted $p$-values implemented by `p.adjust()` with `method = "BH"`. To this end, we create a list of $n=20$ norminal $p$-values and rank them in increasing order:
...
According to [Benjamini-Hochberg](https://www.jstor.org/stable/2346101?seq=1#metadata_info_tab_contents), declaring all $p$-values less than or equal to the largest $p$-value under the line to be significant controls the false discovery rate, in the sense that the expected proportion of false discoveries $\leq \alpha=0.05$. 
