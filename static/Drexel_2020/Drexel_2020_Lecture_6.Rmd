...
```{r}
t.test(cont, fl, var.equal = FALSE)
```
...
`pivot_longer()` takes multiple columns and collapses into key-value pairs, duplicating all other columns as needed.
...
df_long <- df %>%
  pivot_longer(cols = c("low", "high"), #<<
               names_to = "diet", values_to = "glucose")  #<<
slice(df_long, 1:12) # Showing the first 15 rows of data
```
...
```{r, out.width = "45%"}
ggplot(df_long, aes(diet, glucose)) +
  geom_point() +
...
Read in the UK heights dataset:

```{r}
ht <- read.table(file = "heights.txt", header = T, sep = " ") %>%
  mutate(ID = paste0("fam_", 1:nrow(.))) # . = result from piping #<<
```

- Transform `ht` into a long form using `pivot_longer` so that `head()` output of the resulting data looks like the following:

```{r, echo = F}
ht_long <- ht %>%
  pivot_longer(cols = -"ID", names_to = "member", values_to = "height")
head(ht_long)
```
...
- Create the following _violin plot_: the horizontal line segements represent 25%, 50%, 75% quantiles and the red dot represent the mean.

```{r, echo = F, out.width = "45%"}
ht_long %>%
  ggplot(aes(member, height)) +
  geom_violin(draw_quantiles = c(0.25, 0.5, 0.75)) +
  geom_point(stat = "summary", fun = mean, size = 5, color = "red")
```
